...
downsamp1         = cfg_entry; % This is the generic data entry item
downsamp1.name    = 'Downsampling factors (space x spectral)'; % The displayed name
downsamp1.tag     = 'downsampling_factor';       % The name appearing in the harvested job structure. This name must be unique among all items in the val field of the superior node
downsamp1.strtype = 'e';       % No restriction on what type of data is entered. This could be used to restrict input to real numbers, integers ...
downsamp1.num     = [1 2];     % Number of inputs required (2D-array with exactly one row and two column)
downsamp1.val     = {[1 1]};
downsamp1.help    = {'This will be used to downsample the data if required.'}; % help text displayed
...
pca1.val  = {PATmat redo1 downsamp1 npcs1};    % The items that belong to this branch. All items must be filled before this branch can run or produce virtual outputs
