function out = pat_coregistration_run(job)
    ...
    try
        ...
        if ~isfield(PAT.jobsdone,'manualCoregOK') || job.force_redo
            ...
            for c1 = 1:length(PAT.nifti_files)
                ...
                if doColor
                    % skip B-mode only extract PA in raw.pamode
                    if ~(PAT.color.eng(c1)==PAT.color.Bmode) && pat_isVEVOraw(job)
                        % SO2
                        ...
                        if PAT.bModeParam.BmodeDepthOffset <= PAT.PAparam.PaDepthOffset && PAT.bModeParam.BmodeDepth >= PAT.PAparam.PaDepth
                            ...
                            im_PA = padarray(im_PA, [nRowsBottom 0 0 0], val2pad, 'post');
                        end
                        ...
                        if job.generate_figures
                            ...
                            % Save figures
                            pat_save_figs(job, h, 'coreg', scanIdx, c1, 0, 'fig_coregister');
                        end
