...
%  @(#)spm_vol_minc.m	2.9 John Ashburner 01/03/15
...
%****************************************************************
%* A. HUANG set a default signed value in case none is returned *
%****************************************************************
if isempty(signed)
    signed=1; 
else
...
% [Satra] Check if there is a time dimension. The space_names seem to
% shift depending on the existence of the time dimension
timevar = findvar(cdf.dim_array,'time');
if ~isempty(timevar),
    cdf.space_names = {cdf.dim_array(5-[1:3]).name};
else
    cdf.space_names = {cdf.dim_array(4-[1:3]).name};
end;

for j=1:3,
    %	nam    = cdf.dim_array(4-j).name;
    nam = cdf.space_names{j};
...
%[Satra] Multiply the third dimension by the number of
%timepoints/volumes. When spm_read_vols reads it in it will read the
%right number of data points. Then spm_write_4dvol will reshape it
%appropriately and write out the time series. 

if ~isempty(timevar),
    cdf.time_pts = timevar.dim_length;
    dim(3) = dim(3)*cdf.time_pts;
    pinfo = repmat(pinfo,1,cdf.time_pts);
end;
