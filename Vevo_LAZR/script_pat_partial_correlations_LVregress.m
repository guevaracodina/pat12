...
INTERP = true;
clc;
fprintf('Bilateral correlation with global signal regression\n')
...
load('F:\Edgar\Data\PAT_Results\2012-09-07-14-48-55_ctl02\seedRadius14\BPF\GLMfcPAT_LVregressor\PAT.mat')
...
for iROI=1:2:18,
    if INTERP
        % ROI odd
        y = ROIregress{iROI}{c1}(x);
        yi = interp1(x, y, xi);
        ROIregress{iROI}{c1} = yi;
        % ROI even
        y = ROIregress{iROI+1}{c1}(x);
        yi = interp1(x, y, xi);
        ROIregress{iROI+1}{c1} = yi;
    end
    ctrl(1, (iROI+1)/2) = corr(ROIregress{iROI}{c1}(idx)', ROIregress{iROI+1}{c1}(idx)');
end

%% 2012-11-09-16-18-31_ctl03 122:127, 190:193
% load('F:\Edgar\Data\PAT_Results\2012-11-09-16-18-31_ctl03\seedRadius10\PAT.mat')
load('F:\Edgar\Data\PAT_Results\2012-11-09-16-18-31_ctl03\seedRadius10\GLMfcPAT_LVregressor\PAT.mat')
load(PAT.fcPAT.SPM.fnameROIregress)
% scan name
[~, ~, ~, ~, ~, ~, splitStr] = regexp(PAT.input_dir,'\\');
TR = PAT.fcPAT.filtNdown.TR;
...
for iROI=1:2:18,
    ...
    ctrl(2, (iROI+1)/2) = corr(ROIregress{iROI}{c1}(idx)', ROIregress{iROI+1}{c1}(idx)');
end

%% 2012-11-09-16-21-53_ctl01 194:199, 213
% load('F:\Edgar\Data\PAT_Results\2012-11-09-16-21-53_ctl01\seedRadius10\PAT.mat')
load('F:\Edgar\Data\PAT_Results\2012-11-09-16-21-53_ctl01\seedRadius10\GLMfcPAT_LVregressor\PAT.mat')
load(PAT.fcPAT.SPM.fnameROIregress)
% scan name
[~, ~, ~, ~, ~, ~, splitStr] = regexp(PAT.input_dir,'\\');
TR = PAT.fcPAT.filtNdown.TR;
...
for iROI=1:2:18,
    ...
    ctrl(3, (iROI+1)/2) = corr(ROIregress{iROI}{c1}(idx)', ROIregress{iROI+1}{c1}(idx)');
end

%% 2012-11-09-16-23-04_toe08 None
% load('F:\Edgar\Data\PAT_Results\2012-11-09-16-23-04_toe08\seedRadius10\PAT.mat')
load('F:\Edgar\Data\PAT_Results\2012-11-09-16-23-04_toe08\seedRadius10\GLMfcPAT_LVregressor\PAT.mat')
load(PAT.fcPAT.SPM.fnameROIregress)
% scan name
[~, ~, ~, ~, ~, ~, splitStr] = regexp(PAT.input_dir,'\\');
TR = PAT.fcPAT.filtNdown.TR;
...
for iROI=1:2:18,
    ...
    lps(1, (iROI+1)/2) = corr(ROIregress{iROI}{c1}(idx)', ROIregress{iROI+1}{c1}(idx)');
end

%% 2012-11-09-16-23-51_toe09 4:5, 50, 74:76, 107
% load('F:\Edgar\Data\PAT_Results\2012-11-09-16-23-51_toe09\seedRadius10\PAT.mat')
load('F:\Edgar\Data\PAT_Results\2012-11-09-16-23-51_toe09\seedRadius10\GLMfcPAT_LVregressor\PAT.mat')
load(PAT.fcPAT.SPM.fnameROIregress)
% scan name
[~, ~, ~, ~, ~, ~, splitStr] = regexp(PAT.input_dir,'\\');
TR = PAT.fcPAT.filtNdown.TR;
...
for iROI=1:2:18,
    ...
    lps(2, (iROI+1)/2) = corr(ROIregress{iROI}{c1}(idx)', ROIregress{iROI+1}{c1}(idx)');
end
...
load('F:\Edgar\Data\PAT_Results\2012-11-09-16-16-25_toe04\seedRadius10\GLMfcPAT_LVregressor\PAT.mat')
load(PAT.fcPAT.SPM.fnameROIregress)
% scan name
[~, ~, ~, ~, ~, ~, splitStr] = regexp(PAT.input_dir,'\\');
TR = PAT.fcPAT.filtNdown.TR;
...
for iROI=1:2:18,
    ...
    lps(3, (iROI+1)/2) = corr(ROIregress{iROI}{c1}(idx)', ROIregress{iROI+1}{c1}(idx)');
end


%% Stats
mCtrl = mean(ctrl);
semCtrl = std(ctrl)/sqrt(size(ctrl,1));
mLPS = mean(lps);
semLPS = std(lps)/sqrt(size(lps,1));
...
newName = 'PAS2013_bilateral_correlation_LVregress';
set(h,'Name',newName);
